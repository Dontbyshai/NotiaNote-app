...
class LoginSystem {
    static BUNDLE_ID = "LOGIN_SYSTEM_STABLE_V10";
    ...
    /**
     * Main login entry point - NATIVE FETCH SYNC
     */
    static async login(username, password) {
        console.log(`[V10-SYNC] >>> STARTING V10 ATTEMPT [shai563] <<<`);
        ...
        try {
            ...
            // Robust GTK Extraction from all set-cookie headers
            ...
            if (!gtk) {
                console.error("[V10-SYNC] FAILED: No GTK");
                return -1;
            }

            console.log(`[V10-SYNC] GTK Obtained: ${gtk.substring(0, 8)}`);
            ...
            const response = await fetch(`${APIEndpoints.OFFICIAL_API}/v3/login.awp?v=${API_VERSION}`, {
                ...
                headers: {
                    ...
                    'X-Gtk': gtk,
                    'X-Cookie': gtk, // Triple header redundancy
                    'Cookie': cookie,
            ...
            console.log(`[V10-SYNC] SERVER_RESULT: ${code}`);
            ...
            if (code === 200) {
                ...
                console.log("[V10-SYNC] 250! Double Auth required.");
            ...
            console.error("[V10-SYNC] Error:", e.message);
            return -1;
        }
    }
    ...
    static async parseEcoleDirecte(title, id, url, body, success, verbe = "post") {
        ...
        try {
            ...
            console.error(`[V10-SYNC] Error ${title}:`, e.message);
        }
        return -1;
    }
