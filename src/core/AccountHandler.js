...
class AccountHandler {
  ...
  static async login(username, password) {
    ...
    // Demo account
    if (username.length >= 11 && username.substring(0, 11) == "demoaccount") {
    ...
    switch (response.status) {
      case 200:
        ...
        const code = response.data?.code || response.data?.status || 200; // Handling variations
        switch (code) {
          case 200:
            ...
            if (setCookie) {
              var photoCookie = "";
              try {
                // Robust cookie parsing for photo
          ...
          case 250: // Need to confirm identity with security question
            ...
            this.temporary2FAToken = headers["2fa-token"] || headers["2FA-TOKEN"] || headers["x-2fa-token"];
            ...
            status = 3; // 2FA Required (Waits for popup)
            break;
          case 505: // Wrong password
            console.log(`Couldn't connect, wrong password for ${username}`);
            // Force reset credentials/cookies if this happens to clear bad state
            await StorageHandler.saveData("gtk", null);
            await StorageHandler.saveData("cookies", null);
            break;
          default:
  ...
  static async _saveAccount(account, token) {
    ...
    if (accountType == "E") {
      ...
      accountData = {
        ...
        grade: grade,
        photoURL: photoURL,
        modules: account.modules || [], // Save modules
        profile: account.profile || {}, // Save full profile data
      };
    } else {
      // Parent account
  ...
  static async parseEcoleDirecte(title, accountID, url, payload, successCallback, verbe = "get", accountIDForToken = 0) {
    ...
    finalURL.searchParams.set("v", "4.75.0");
    ...
    // fetchED handles the parsing already for iOS/SecureFetch (returns parsed data in .data if possible)
    var response = responseED ? {
      status: 200,
      data: responseED.data, // fetchED normalized this
